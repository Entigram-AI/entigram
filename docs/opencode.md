...
If OpenCode Desktop cannot find `etg`, use the absolute path that your shell
resolves. Desktop apps on macOS often do not inherit the same `PATH` as your
interactive terminal, especially when `etg` is installed in a project virtualenv
or `pipx` environment.
...
Then put that path in `.opencode.jsonc`:
...
This is especially common when `etg` comes from a project virtualenv, `pipx`, or
a shell startup file that OpenCode does not load.

For a source checkout using this repository's virtualenv, the command usually
looks like:
...
```jsonc
{
  ...
  "mcp": {
    "entigram": {
      "type": "local",
      "command": [
        "/Users/dnyabuti/Documents/workspace/entigram/.venv/bin/etg",
        "serve",
        "--dir",
        "."
      ],
