# Changelog

## Unreleased

### Features

* add 2.8.0 task-envelope workflow to structure and require acceptance for LLM action intents before execution
* add out-of-the-box expectation guard for pre-handoff agent verification
* add `etg serve` MCP server with schema-gated alignment and conflict tools
* publish agent discoverability through `ai-catalog.json`
* add privacy-preserving `etg usage` estimates for static context and observed CLI/MCP traffic
* add reversible workspace `pause`, `resume`, and archive-first `eject` controls

### Bug Fixes

* harden SQLite ledger concurrency with WAL mode and busy timeouts
* close warning-producing registry, broker, router, and hydration resources

## [2.23.1](https://github.com/Entigram-AI/entigram/compare/v2.23.0...v2.23.1) (2026-09-25)

* Maintenance release with dependency and internal updates.

## [2.23.0](https://github.com/Entigram-AI/entigram/compare/v2.22.1...v2.23.0) (2026-09-25)

* Maintenance release with dependency and internal updates.

## [2.22.1](https://github.com/Entigram-AI/entigram/compare/v2.22.0...v2.22.1) (2026-09-25)

### Bug Fixes

* scope lifecycle snapshots to workspaces ([#112](https://github.com/Entigram-AI/entigram/issues/112)) ([6952b6d](https://github.com/Entigram-AI/entigram/commit/6952b6de3f4949622c27bee740262f3fb06a6c59))

## [2.22.0](https://github.com/Entigram-AI/entigram/compare/v2.21.4...v2.22.0) (2026-09-25)

* Maintenance release with dependency and internal updates.

## [2.21.4](https://github.com/Entigram-AI/entigram/compare/v2.21.3...v2.21.4) (2026-09-25)

### Bug Fixes

* proxy nested workspace artifacts ([#111](https://github.com/Entigram-AI/entigram/issues/111)) ([0cd762a](https://github.com/Entigram-AI/entigram/commit/0cd762aca1b0dbbe6cebdb5c3562f97adba83c14))

## [2.21.3](https://github.com/Entigram-AI/entigram/compare/v2.21.2...v2.21.3) (2026-09-16)

* Maintenance release with dependency and internal updates.

## [2.21.2](https://github.com/Entigram-AI/entigram/compare/v2.21.1...v2.21.2) (2026-09-16)

### Bug Fixes

* bound unanchored artifact discovery and add .etgignore support ([#109](https://github.com/Entigram-AI/entigram/issues/109)) ([e57c9d5](https://github.com/Entigram-AI/entigram/commit/e57c9d5b1aa201addb6f39d7b212d22525d5d8b5))

## [2.21.1](https://github.com/Entigram-AI/entigram/compare/v2.21.0...v2.21.1) (2026-09-16)


### Bug Fixes

* loosen active change budget to 15 files with warning at 5 ([#106](https://github.com/Entigram-AI/entigram/issues/106)) ([e611227](https://github.com/Entigram-AI/entigram/commit/e611227dc1bdb0c1b01d977133c40b3910a3df1e))

## [2.21.0](https://github.com/Entigram-AI/entigram/compare/v2.20.0...v2.21.0) (2026-09-15)


### Features

* dispatch governed agent tasks ([#103](https://github.com/Entigram-AI/entigram/issues/103)) ([c3e38e2](https://github.com/Entigram-AI/entigram/commit/c3e38e24012d2aca0ce5197b9fd65d082edb4f18))

## [2.20.0](https://github.com/Entigram-AI/entigram/compare/v2.19.0...v2.20.0) (2026-09-15)


### Features

* strengthen governed task lifecycle ([#101](https://github.com/Entigram-AI/entigram/issues/101)) ([6943f8e](https://github.com/Entigram-AI/entigram/commit/6943f8e2b099a977f5f2a79b863d8c86330f5761))

## [2.19.0](https://github.com/Entigram-AI/entigram/compare/v2.18.0...v2.19.0) (2026-09-13)


### Features

* add explicit workspace links ([#99](https://github.com/Entigram-AI/entigram/issues/99)) ([9d79346](https://github.com/Entigram-AI/entigram/commit/9d793467f0efca0857cd5149b2a1e8f3c214db88))

## [2.18.0](https://github.com/Entigram-AI/entigram/compare/v2.17.3...v2.18.0) (2026-09-11)


### Features

* harden Sentinel policy evidence and retire AgentBeats manifest ([#97](https://github.com/Entigram-AI/entigram/issues/97)) ([7ed16ea](https://github.com/Entigram-AI/entigram/commit/7ed16eaea0847af125dc239cb17fb692ff004dcb))

## [2.17.3](https://github.com/Entigram-AI/entigram/compare/v2.17.2...v2.17.3) (2026-09-10)


### Bug Fixes

* admit grounded policy section citations ([#95](https://github.com/Entigram-AI/entigram/issues/95)) ([de0e412](https://github.com/Entigram-AI/entigram/commit/de0e412b862555f7e73092c25b76c2edefb8ee88))

## [2.17.2](https://github.com/Entigram-AI/entigram/compare/v2.17.1...v2.17.2) (2026-09-10)


### Bug Fixes

* infer explicit tool completion contracts ([#93](https://github.com/Entigram-AI/entigram/issues/93)) ([f40ea6d](https://github.com/Entigram-AI/entigram/commit/f40ea6da5dd232940a2068f07355e57c0c268e92))

## [2.17.1](https://github.com/Entigram-AI/entigram/compare/v2.17.0...v2.17.1) (2026-09-10)


### Bug Fixes

* complete hydrated Sentinel decisions ([#91](https://github.com/Entigram-AI/entigram/issues/91)) ([8d3a78c](https://github.com/Entigram-AI/entigram/commit/8d3a78c78abf9347673194e708fbeb0657ab1858))

## [2.17.0](https://github.com/Entigram-AI/entigram/compare/v2.16.0...v2.17.0) (2026-09-10)


### Features

* add Sentinel hydration diagnostics ([#89](https://github.com/Entigram-AI/entigram/issues/89)) ([c748acc](https://github.com/Entigram-AI/entigram/commit/c748acc5a098f170547778c9ea4807660bbbb871))

## [2.16.0](https://github.com/Entigram-AI/entigram/compare/v2.15.0...v2.16.0) (2026-09-10)


### Features

* hydrate session policy mediation ([#87](https://github.com/Entigram-AI/entigram/issues/87)) ([2b83968](https://github.com/Entigram-AI/entigram/commit/2b839680f9f1ad9905f22ab2ffc5d951aae6c3a5))

## [2.15.0](https://github.com/Entigram-AI/entigram/compare/v2.14.0...v2.15.0) (2026-09-10)


### Features

* bind tool proposals to policy references ([#85](https://github.com/Entigram-AI/entigram/issues/85)) ([14f17c7](https://github.com/Entigram-AI/entigram/commit/14f17c7a7b0960da73a88e47b737f09230e79c6c))

## [2.14.0](https://github.com/Entigram-AI/entigram/compare/v2.13.1...v2.14.0) (2026-09-10)


### Features

* admit native tool proposals ([#83](https://github.com/Entigram-AI/entigram/issues/83)) ([99addb9](https://github.com/Entigram-AI/entigram/commit/99addb9292f8d7ccb57d600fd66f30a963af0baa))

## [2.13.1](https://github.com/Entigram-AI/entigram/compare/v2.13.0...v2.13.1) (2026-09-10)


### Bug Fixes

* preserve Sentinel action plan order ([#81](https://github.com/Entigram-AI/entigram/issues/81)) ([c52b15d](https://github.com/Entigram-AI/entigram/commit/c52b15de9f65974cb3ed2afe874de0944baed607))

## [2.13.0](https://github.com/Entigram-AI/entigram/compare/v2.12.2...v2.13.0) (2026-09-10)


### Features

* add Sentinel policy admission ([#79](https://github.com/Entigram-AI/entigram/issues/79)) ([f2de391](https://github.com/Entigram-AI/entigram/commit/f2de39107440dfef01f9bfab885db6de90d5994b))

## [2.12.2](https://github.com/Entigram-AI/entigram/compare/v2.12.1...v2.12.2) (2026-09-10)


### Bug Fixes

* add Sentinel health check client ([#77](https://github.com/Entigram-AI/entigram/issues/77)) ([6b3827e](https://github.com/Entigram-AI/entigram/commit/6b3827e45ddc8ee7067904bf05ce83051c764a2d))

## [2.12.1](https://github.com/Entigram-AI/entigram/compare/v2.12.0...v2.12.1) (2026-09-10)


### Bug Fixes

* use OpenAI for Sentinel responses ([#75](https://github.com/Entigram-AI/entigram/issues/75)) ([84c2ddb](https://github.com/Entigram-AI/entigram/commit/84c2ddbb230310979c65fea2951b5e071fb42088))

## [2.12.0](https://github.com/Entigram-AI/entigram/compare/v2.11.0...v2.12.0) (2026-09-10)


### Features

* add PiBench Sentinel bootstrap ([#73](https://github.com/Entigram-AI/entigram/issues/73)) ([42bdb83](https://github.com/Entigram-AI/entigram/commit/42bdb838a5edf2b65bf0715ee5530f959baf366f))

## [2.11.0](https://github.com/Entigram-AI/entigram/compare/v2.10.0...v2.11.0) (2026-09-10)


### Features

* add Entigram Sentinel AgentBeats manifest ([#71](https://github.com/Entigram-AI/entigram/issues/71)) ([fb19099](https://github.com/Entigram-AI/entigram/commit/fb190992fcc3c88c4ea7fb1add5b44cbeca60229))

## [2.10.0](https://github.com/Entigram-AI/entigram/compare/v2.9.0...v2.10.0) (2026-09-09)


### Features

* add normalized action decision events ([#69](https://github.com/Entigram-AI/entigram/issues/69)) ([5ccc277](https://github.com/Entigram-AI/entigram/commit/5ccc2777db2573b92b0f9abc0f76e31766b790aa))

## [2.9.0](https://github.com/Entigram-AI/entigram/compare/v2.8.0...v2.9.0) (2026-09-08)


### Features

* add Git-native governed collaboration ([#67](https://github.com/Entigram-AI/entigram/issues/67)) ([4325a50](https://github.com/Entigram-AI/entigram/commit/4325a50af6e1ec1e0b14997780bb6ea7c7a0b2c8))

## [2.8.0](https://github.com/Entigram-AI/entigram/compare/v2.7.0...v2.8.0) (2026-09-07)


### Features

* add proposal-first task envelopes for 2.8 ([#65](https://github.com/Entigram-AI/entigram/issues/65)) ([8024328](https://github.com/Entigram-AI/entigram/commit/80243287f00a8272c471b5579f569630685d177f))

## [2.7.0](https://github.com/Entigram-AI/entigram/compare/v2.6.0...v2.7.0) (2026-09-06)


### Features

* add deterministic task expectation envelope ([#63](https://github.com/Entigram-AI/entigram/issues/63)) ([755e704](https://github.com/Entigram-AI/entigram/commit/755e7046551a8016855ff6e5a30cd04bea94df91))

## [2.6.0](https://github.com/Entigram-AI/entigram/compare/v2.5.1...v2.6.0) (2026-09-06)


### Features

* require deterministic task preparation before governed writes ([#61](https://github.com/Entigram-AI/entigram/issues/61)) ([3d8c4b1](https://github.com/Entigram-AI/entigram/commit/3d8c4b18aec2c25b7176073abb8064f93f931307))

## [2.5.1](https://github.com/Entigram-AI/entigram/compare/v2.5.0...v2.5.1) (2026-09-05)


### Bug Fixes

* harden patch admission evidence ([#59](https://github.com/Entigram-AI/entigram/issues/59)) ([d53108a](https://github.com/Entigram-AI/entigram/commit/d53108aa4059a6b8cd8f56f47e1a02268e14ef54))

## [2.5.0](https://github.com/Entigram-AI/entigram/compare/v2.4.0...v2.5.0) (2026-09-04)


### Features

* add opt-in patch admission inspection ([#56](https://github.com/Entigram-AI/entigram/issues/56)) ([5cb2abb](https://github.com/Entigram-AI/entigram/commit/5cb2abba01a6c99fca1307118119b03f0c213820))

## [2.4.0](https://github.com/Entigram-AI/entigram/compare/v2.3.0...v2.4.0) (2026-09-04)


### Features

* remove experimental model routing MVP to restore v2.2.0 state ([#54](https://github.com/Entigram-AI/entigram/issues/54)) ([ba4adab](https://github.com/Entigram-AI/entigram/commit/ba4adab4628671baf27b16b65d9597d30e36f2fc))

## [2.3.0](https://github.com/Entigram-AI/entigram/compare/v2.2.0...v2.3.0) (2026-09-03)


### Features

* add deterministic model routing MVP ([#52](https://github.com/Entigram-AI/entigram/issues/52)) ([44cfee8](https://github.com/Entigram-AI/entigram/commit/44cfee8b637ecf903f9b0e4805fb51202135b26d))

## [2.2.0](https://github.com/Entigram-AI/entigram/compare/v2.1.0...v2.2.0) (2026-08-31)


### Features

* add Entigram client and recommission recovery ([#48](https://github.com/Entigram-AI/entigram/issues/48)) ([510e420](https://github.com/Entigram-AI/entigram/commit/510e42040b076723e5585cb3ba67c1f83e54aec8))


### Bug Fixes

* follow up post-merge review findings ([#50](https://github.com/Entigram-AI/entigram/issues/50)) ([8a85877](https://github.com/Entigram-AI/entigram/commit/8a8587770433d802d488a177fee913e06256b5f2))

## [2.1.0](https://github.com/Entigram-AI/entigram/compare/v2.0.0...v2.1.0) (2026-08-16)


### Features

* add agent orchestration hibernation ([cc58dc4](https://github.com/Entigram-AI/entigram/commit/cc58dc44694bf7c872a557fe43f28cd2743b776a))
* add agent orchestration hibernation ([5e5b353](https://github.com/Entigram-AI/entigram/commit/5e5b353b3dead4ae622949493a9e70df822e92e5))
* add assessment package catalog and request-access monetization hook ([243fcc5](https://github.com/Entigram-AI/entigram/commit/243fcc570486771c2221bf1e16fe3f7d304dff27))
* add audit bundle and maintainer docs ([5ec31dd](https://github.com/Entigram-AI/entigram/commit/5ec31dd1e064c981586c8237dedc9fb6904be267))
* add Cloudflare Claude proxy launcher ([2d0c9bf](https://github.com/Entigram-AI/entigram/commit/2d0c9bf92cfbf50cfa1f93439db2fdc8d06295b4))
* add Cloudflare Claude proxy launcher ([e4751ee](https://github.com/Entigram-AI/entigram/commit/e4751ee155ad50be08366f5316aa4854b77aff88))
* add discovery review and signed package catalog ([3738db5](https://github.com/Entigram-AI/entigram/commit/3738db5ad5efe4775268f7576c07cd092ef9b6b2))
* add discovery review and signed package catalog ([ec7acf3](https://github.com/Entigram-AI/entigram/commit/ec7acf3b570a8f5aafd84ba358041871922d2144))
* add etg merge ([f7088f6](https://github.com/Entigram-AI/entigram/commit/f7088f62b3e825640037f95fc92759d89bc5845a))
* add etg merge ([d35710d](https://github.com/Entigram-AI/entigram/commit/d35710d04f0944d106a3bbc181f63b06669c4e59))
* add expectation guard for pre-handoff agent verification ([41a125f](https://github.com/Entigram-AI/entigram/commit/41a125f6138eca23d29d50c66ee77fdff7b06ff3))
* add model gate feedback retries ([b1029bb](https://github.com/Entigram-AI/entigram/commit/b1029bbdb93b6367365d75fe2413ab88bb2cfd29))
* add model gate feedback retries ([a62a667](https://github.com/Entigram-AI/entigram/commit/a62a667ef5f557f04f88daec91b19435abf82a33))
* add modern Entigram logo assets ([83cf77c](https://github.com/Entigram-AI/entigram/commit/83cf77ce44fe7fd41aace1999b6fb2966aafc1ad))
* add modern Entigram logo assets ([f68e228](https://github.com/Entigram-AI/entigram/commit/f68e22813ef879b503ff5505e1573b220b6f726b))
* add Ollama launch option selection ([f40be78](https://github.com/Entigram-AI/entigram/commit/f40be78cf06c06a57e1e8c1e6b7c3548d1d23b67))
* add structured halt event json ([988dc12](https://github.com/Entigram-AI/entigram/commit/988dc120eecc7a10485a103c706f18bf0fef49d0))
* add structured halt event json ([0647e1c](https://github.com/Entigram-AI/entigram/commit/0647e1cecd29e2c1ac36f7859eaf0ee1df9a2108))
* add v2 action admission governance ([#45](https://github.com/Entigram-AI/entigram/issues/45)) ([f9d0fbf](https://github.com/Entigram-AI/entigram/commit/f9d0fbf3fc5953e4773c872f9363ec5cc25899fd))
* add WebSocket panel bridge for Agent-Hosted Panels ([07b85b3](https://github.com/Entigram-AI/entigram/commit/07b85b341bd3e5a8210e63321bb42294f8682306))
* add WebSocket panel bridge for Agent-Hosted Panels ([8527ee7](https://github.com/Entigram-AI/entigram/commit/8527ee73a48f6736ea0bad1344e91f7d3a4e7c65))
* add workspace usage and lifecycle controls ([2a5ece4](https://github.com/Entigram-AI/entigram/commit/2a5ece44dd0d39534183413b6bd7a79315c9cee2))
* add workspace usage and lifecycle controls ([7619ee9](https://github.com/Entigram-AI/entigram/commit/7619ee9315c77fbcf0b26bf194c1dad801832e32))
* add zero-trust MCP server ([d7d7e7a](https://github.com/Entigram-AI/entigram/commit/d7d7e7a240fe5e3df8582fe2933d5668813c4162))
* add zero-trust MCP server ([a949bc7](https://github.com/Entigram-AI/entigram/commit/a949bc73b02c992d0e91b73845dd14efb77efe54))
* auto-select Cloudflare coding models ([5ff4552](https://github.com/Entigram-AI/entigram/commit/5ff4552dcf0ada1906e1f2d443c44dd08d01a936))
* close indispensability gap — learnings, trust score, E2T mapping, session proposals ([9798f97](https://github.com/Entigram-AI/entigram/commit/9798f97d575493966a3e31f8167d6634bcb33697))
* commissioner companion features — evidence ledger, delivery snapshots, improvement proposals, Blocked state, CLI deliver/resolve/improve ([9bceedb](https://github.com/Entigram-AI/entigram/commit/9bceedb1e36e9711ab4c3c39d7a6bbf0e43d9824))
* deterministic ontology generation and change impact analysis ([39780f5](https://github.com/Entigram-AI/entigram/commit/39780f5036819753c9bae36b0f4fb1f040d11840))
* deterministic ontology generation and change impact analysis ([045fb10](https://github.com/Entigram-AI/entigram/commit/045fb10acae548dfcd707c23fb97ba8db3ff8e44))
* enforce adapters for workspace agents ([#37](https://github.com/Entigram-AI/entigram/issues/37)) ([9d1302b](https://github.com/Entigram-AI/entigram/commit/9d1302be631ee8f648d774496c76be2831f3daf7))
* enforce agent lifecycle check-ins ([#35](https://github.com/Entigram-AI/entigram/issues/35)) ([8320e21](https://github.com/Entigram-AI/entigram/commit/8320e2154dc1c8cf868e4c3182e405e7595332d3))
* GitHub issue links for package requests + demand tracking ([95b4605](https://github.com/Entigram-AI/entigram/commit/95b4605ad2834d0c866581b5da25c1f1279d9f42))
* governed security assessments, tech-aware advisories, and UI removal ([79a161b](https://github.com/Entigram-AI/entigram/commit/79a161b6334595b86ae6949eb19d00c39cf09de7))
* harden Cloudflare backend and agent governance ([9a68c83](https://github.com/Entigram-AI/entigram/commit/9a68c8320c925b8b9159f2a3a477e41950bed563))
* harden governed security assessment boundaries ([6d1fdc8](https://github.com/Entigram-AI/entigram/commit/6d1fdc8e79146ef4e16c9b4fb24e8496b729d926))
* harden governed security assessment boundaries ([de42a50](https://github.com/Entigram-AI/entigram/commit/de42a50bb1c07f4defa4f31d56e79ce6fb307934))
* harden MCP gate contract ([6828668](https://github.com/Entigram-AI/entigram/commit/6828668fced9e861b9218e653efccbd69cbf08e4))
* implement Cloud API package fetching using ENTIGRAM_TOKEN ([8bb6626](https://github.com/Entigram-AI/entigram/commit/8bb6626af19931e5b5688290fe873443549ef854))
* implement Cloud API package fetching using ENTIGRAM_TOKEN ([41783c4](https://github.com/Entigram-AI/entigram/commit/41783c4125f13dc4c4b558f7aadfe50926ece8eb))
* improve agent discoverability ([#34](https://github.com/Entigram-AI/entigram/issues/34)) ([ce270d6](https://github.com/Entigram-AI/entigram/commit/ce270d6bb2813e5bd59343904f2e069f9e544e2c))
* inject RelationalAlgebraGuard into etg_propose_alignment MCP handler ([090bb2c](https://github.com/Entigram-AI/entigram/commit/090bb2cd0e8a8e5b447f09a8f728be67cd39f0ef))
* install multi-agent PR review skill (moltenbits/claude-review pattern) ([b1eb279](https://github.com/Entigram-AI/entigram/commit/b1eb279d2cc6dddd60f5600aedf626c0f90d8e81))
* make agent governance portable ([b152b20](https://github.com/Entigram-AI/entigram/commit/b152b209b089b95e347b7bc75d25e1966db93a2a))
* make hydrate concise by default ([2b6fe0c](https://github.com/Entigram-AI/entigram/commit/2b6fe0cfda454a12b5c0100a70bf0f1d578e1a88))
* make hydrate concise by default ([29c0ea2](https://github.com/Entigram-AI/entigram/commit/29c0ea2c0157cdc4353015138cec40f7b1b80578))
* model canonical cross-agent governance policy ([fb30227](https://github.com/Entigram-AI/entigram/commit/fb30227f0bffd8cf71d6f2e54e3e82ab9f4199b5))
* out-of-the-box expectation guard for pre-handoff agent verification ([e558d63](https://github.com/Entigram-AI/entigram/commit/e558d6358cf4be4480f436b721fe6af01af4e7d5))
* package 6 multi-agent skills with entigram ([a53739b](https://github.com/Entigram-AI/entigram/commit/a53739b1be833b69f14aff3c1455cd414b74d141))
* publish community packages through standard registry ([#27](https://github.com/Entigram-AI/entigram/issues/27)) ([ac1a6d2](https://github.com/Entigram-AI/entigram/commit/ac1a6d26d0a68fdc97b6da3c9a41a8f41e074833))
* publish complete MCP tool contracts ([#44](https://github.com/Entigram-AI/entigram/issues/44)) ([8d0b798](https://github.com/Entigram-AI/entigram/commit/8d0b798c0c86b19428e4e4ca8ea1c1a5705516e9))
* security assessments, multi-agent skills, and package request flow ([caf389c](https://github.com/Entigram-AI/entigram/commit/caf389c3e6c8573aae693c5f25c8a98b55e9eddb))
* sign audit bundles and clarify versions ([131bddf](https://github.com/Entigram-AI/entigram/commit/131bddff5fdc2e7f9b9c00088d84bb9481722813))


### Bug Fixes

* add debug logging to suppression loader instead of silent swallow ([abe4742](https://github.com/Entigram-AI/entigram/commit/abe4742a3f5b2ed1144461dd50b1f4f3bd25f382))
* address P0-P2 findings from multi-agent review + rewrite skill ([c205276](https://github.com/Entigram-AI/entigram/commit/c2052762cacb818d6b9ccc0592dd9d2283b7301a))
* address P1/P2 findings from multi-agent review ([b4b124d](https://github.com/Entigram-AI/entigram/commit/b4b124d23eb7f6b36d324de014ababae2bd470e5))
* align Homebrew formula Python runtime ([7430531](https://github.com/Entigram-AI/entigram/commit/7430531a4fc27d923380e8f741b261cad1945da2))
* align Homebrew formula Python runtime ([615cbd2](https://github.com/Entigram-AI/entigram/commit/615cbd27b6cd53e57c57c00d6acd410dae300e0e))
* align Python support with MCP dependency ([f6e2309](https://github.com/Entigram-AI/entigram/commit/f6e2309bbb6a5050df07a1fb7b0179367b3f453f))
* align tests with federated routing guards ([f365811](https://github.com/Entigram-AI/entigram/commit/f365811b727dadf340f1a2b9012d198c5c93d32f))
* align tests with federated routing guards ([4999f87](https://github.com/Entigram-AI/entigram/commit/4999f87f441f443a908521cd06aa2af321bda30c))
* always raise flags — elevate advisories to warning, add suppression with rationale ([c5b9b81](https://github.com/Entigram-AI/entigram/commit/c5b9b8102d5d09e4d2aac67bc238a84d5f436a7f))
* avoid Claude slash model interception ([1885cff](https://github.com/Entigram-AI/entigram/commit/1885cffb5754bdb1afd0093e3e2c0398acc3631e))
* broaden homebrew formula detection ([15149c5](https://github.com/Entigram-AI/entigram/commit/15149c5b40594406515731ca4bea9c33c963154e))
* broaden homebrew formula detection ([f17eb13](https://github.com/Entigram-AI/entigram/commit/f17eb1349e53a06a50fc927634a2e2655cb5ed73))
* close CodeQL input handling alerts ([e4455ce](https://github.com/Entigram-AI/entigram/commit/e4455ce605003687ecc9401590f38a2bd2af03f3))
* close governed delivery review gaps ([23bc947](https://github.com/Entigram-AI/entigram/commit/23bc9476672a98c6c476cebb43feee8196483561))
* correct pre-handoff governance order ([056e009](https://github.com/Entigram-AI/entigram/commit/056e0094c14eb7875467137c2c5d59296ff57f54))
* direct access requests to developer@entigram.com ([094f1f8](https://github.com/Entigram-AI/entigram/commit/094f1f86459b3f4ddbfed09088871fd09cd6cb98))
* dynamically read version from pyproject.toml in tests ([ed07091](https://github.com/Entigram-AI/entigram/commit/ed07091eb98e50e4d1f98480904951ae38167e42))
* eliminate cryptography dependency for Homebrew builds ([7604ebb](https://github.com/Entigram-AI/entigram/commit/7604ebbce5b23aef39e0cf3bfd6d2a6f2b9bed8e))
* eliminate PyPI index race condition in homebrew release script ([04adeaa](https://github.com/Entigram-AI/entigram/commit/04adeaa6eb6100b900be4d3a8e563e9828812c94))
* emit proactive security advisories for detected workspace technologies ([2c80f5a](https://github.com/Entigram-AI/entigram/commit/2c80f5a2b903b4fc0851a893b8ad055006e4b50e))
* filter cryptography from poet resources and inject Homebrew dependency ([3197f1b](https://github.com/Entigram-AI/entigram/commit/3197f1b84ff38a9a091c2bd145b23ca65a60ac6a))
* filter cryptography from poet resources and inject Homebrew dependency ([ebda4d1](https://github.com/Entigram-AI/entigram/commit/ebda4d16b27cdf090a681f1c82a68e29efcd339d))
* filter pydantic and rpds-py rust dependencies ([a3f7aa2](https://github.com/Entigram-AI/entigram/commit/a3f7aa24bf3c2b078c0c8c1a5467addcc7699634))
* filter pydantic and rpds-py rust dependencies ([1ff8234](https://github.com/Entigram-AI/entigram/commit/1ff8234a7d9752394cdec7ba24913a1be5d06dab))
* generate brew-style-compliant formulae ([393a54f](https://github.com/Entigram-AI/entigram/commit/393a54f4d31e8ad6424c3579df19d340ee4c911d))
* generate brew-style-compliant formulae ([38790b6](https://github.com/Entigram-AI/entigram/commit/38790b6b669bc8e7cd409a76208cbb5b1684bfdc))
* generate homebrew formula resources using poet ([858eb2a](https://github.com/Entigram-AI/entigram/commit/858eb2a87b01487b0c9df267f8e5423943befcb3))
* generate homebrew formula resources using poet ([6a30a5e](https://github.com/Entigram-AI/entigram/commit/6a30a5ef08121a8a804927ab8f6bab350aeb0d06))
* harden governed delivery paths ([8924bab](https://github.com/Entigram-AI/entigram/commit/8924bab73868ead0292349617fe110972a1708d5))
* harden governed delivery paths ([5eeea8f](https://github.com/Entigram-AI/entigram/commit/5eeea8f1b8bd780ec1ac883c8edf8dae4811d46b))
* harden Homebrew resource filtering ([80363e6](https://github.com/Entigram-AI/entigram/commit/80363e65e850c747e5e45d5cc46417007653e9d6))
* harden impact analysis and alignment precedence ([e4b4f31](https://github.com/Entigram-AI/entigram/commit/e4b4f31d83e14d5e6ccd3c96e613dc06b73dde41))
* include setuptools in Homebrew resources ([5a8d7fa](https://github.com/Entigram-AI/entigram/commit/5a8d7fa53435362b53a59230f386babc3844d224))
* integrate assess into hydration, expand frameworks beyond OWASP, add package recs ([ee92c7e](https://github.com/Entigram-AI/entigram/commit/ee92c7ea0a13783776fedb3a955d3615cedca80b))
* make adapter fields optional in catalog audit for domain-only packages ([4152f8a](https://github.com/Entigram-AI/entigram/commit/4152f8a8725a3b325282e05548b97cd89f5c7a88))
* make adapter_module, source_kinds, adapters optional in catalog audit ([eac6887](https://github.com/Entigram-AI/entigram/commit/eac6887534ffc803ab4386aff1dc4e86483a8fe6))
* make community package installs self-guiding ([#30](https://github.com/Entigram-AI/entigram/issues/30)) ([058b202](https://github.com/Entigram-AI/entigram/commit/058b20255b2b51e1de11d16c91fdb00cc01194c8))
* model release PR governance rules ([bc92081](https://github.com/Entigram-AI/entigram/commit/bc92081560845df885260217deaab6e72764ee55))
* model release PR governance rules ([7dc2b67](https://github.com/Entigram-AI/entigram/commit/7dc2b67cba8cac3c4d458feb0927654204fa3f35))
* name initialized schema for Entigram ([#40](https://github.com/Entigram-AI/entigram/issues/40)) ([4ae679d](https://github.com/Entigram-AI/entigram/commit/4ae679d97c60fcee1f2a432d21f904ff7de1152d))
* normalize Cloudflare proxy message content ([d1914b0](https://github.com/Entigram-AI/entigram/commit/d1914b08f823e8c222c96d4eb4fc9686a8e3e37e))
* remove stale entigram.ui package refs breaking CI build ([27a77ac](https://github.com/Entigram-AI/entigram/commit/27a77acaed9f25dcce97d790ff8b156658ffbab6))
* remove vulnerable ecdsa dependency ([1bba9ab](https://github.com/Entigram-AI/entigram/commit/1bba9ab27d0bc91d68721541cba3e4d6372c8174))
* remove vulnerable ecdsa dependency ([e3a1763](https://github.com/Entigram-AI/entigram/commit/e3a17631d90e1d8d49582df4df7cea7785353907))
* replace cryptography with pure python ecdsa to resolve Homebrew sandbox build failures ([8728e89](https://github.com/Entigram-AI/entigram/commit/8728e893b0fb8c2de25e50d2c3685e79f893a4cd))
* resolve homebrew formula path dynamically ([4817f56](https://github.com/Entigram-AI/entigram/commit/4817f5618a66b500e251df4d4698d619eb848a60))
* resolve homebrew formula path dynamically ([4f64828](https://github.com/Entigram-AI/entigram/commit/4f64828a9fe09d3434ea8f13ca91ac4ca40ba97e))
* support pyproject-only release versioning ([2985462](https://github.com/Entigram-AI/entigram/commit/2985462a425191a6b9add0636518b3f57bc2de50))
* support pyproject-only release versioning ([eaeecee](https://github.com/Entigram-AI/entigram/commit/eaeecee22d58a9043086006f158d546d9aebd060))
* sync requirements.txt for CI build ([903b100](https://github.com/Entigram-AI/entigram/commit/903b100356b56c9c298fb9144cda63a794d16243))
* update hardcoded version 1.6.0 to 1.7.0 in tests ([aed13a8](https://github.com/Entigram-AI/entigram/commit/aed13a8e001e6464f05d961f5d2395260daa369c))
* update homebrew tap from pypi metadata ([4f3c5b2](https://github.com/Entigram-AI/entigram/commit/4f3c5b262e76cc96033106f816ba5128add66276))
* update homebrew tap from pypi metadata ([9c3a668](https://github.com/Entigram-AI/entigram/commit/9c3a668e364a00b13406d04f2192d045d5fe3799))
* use local repo root to avoid PyPI index race condition ([d2b329b](https://github.com/Entigram-AI/entigram/commit/d2b329b0b6cb687b4eb68c497120ab94afad8ae7))
* wait for PyPI sdist metadata ([108e19b](https://github.com/Entigram-AI/entigram/commit/108e19b0dda50c50bc8ae15fe89417ce04a6c168))
* wait for PyPI sdist metadata ([8fe7b28](https://github.com/Entigram-AI/entigram/commit/8fe7b28de9a3d545b04ebfc989de86ea044f7b4c))


### Documentation

* add M8ven verification badge ([#42](https://github.com/Entigram-AI/entigram/issues/42)) ([8f4e841](https://github.com/Entigram-AI/entigram/commit/8f4e8418c15e7358ed6044fc80333808766c1ba0))
* add opencode integration guidance ([2ccedcd](https://github.com/Entigram-AI/entigram/commit/2ccedcdabae57fbe115671363aab3fafc8af1789))
* add OpenCode integration guidance ([b09706c](https://github.com/Entigram-AI/entigram/commit/b09706c273aa569c13d5ea6ac6be17b2f906907b))
* add workspace alignment check to agent initialization step ([1aa7ef8](https://github.com/Entigram-AI/entigram/commit/1aa7ef86c5a7a2a57a107ead3002ed42df8324da))
* clarify user and agent workflows ([#32](https://github.com/Entigram-AI/entigram/issues/32)) ([3cdd3ae](https://github.com/Entigram-AI/entigram/commit/3cdd3ae02a2e9d7f7b42b6b0d458b07f6503a9fd))
* deduplicate 1.11.0 changelog entry ([67c6a12](https://github.com/Entigram-AI/entigram/commit/67c6a127a3c4b84ed147a46815619c792710f9ee))
* publish Entigram workspace standard ([428c55b](https://github.com/Entigram-AI/entigram/commit/428c55bfe432a9ff55a6223811f942b0700b24b0))
* publish workspace standard ([6ed435d](https://github.com/Entigram-AI/entigram/commit/6ed435d4645bf7527ed9889d7c2180c928cbb138))

## [2.0.0](https://github.com/Entigram-AI/entigram/compare/v1.25.0...v2.0.0) (2026-08-16)


### Features

* add agent orchestration hibernation ([cc58dc4](https://github.com/Entigram-AI/entigram/commit/cc58dc44694bf7c872a557fe43f28cd2743b776a))
* add agent orchestration hibernation ([5e5b353](https://github.com/Entigram-AI/entigram/commit/5e5b353b3dead4ae622949493a9e70df822e92e5))
* add assessment package catalog and request-access monetization hook ([243fcc5](https://github.com/Entigram-AI/entigram/commit/243fcc570486771c2221bf1e16fe3f7d304dff27))
* add audit bundle and maintainer docs ([5ec31dd](https://github.com/Entigram-AI/entigram/commit/5ec31dd1e064c981586c8237dedc9fb6904be267))
* add Cloudflare Claude proxy launcher ([2d0c9bf](https://github.com/Entigram-AI/entigram/commit/2d0c9bf92cfbf50cfa1f93439db2fdc8d06295b4))
* add Cloudflare Claude proxy launcher ([e4751ee](https://github.com/Entigram-AI/entigram/commit/e4751ee155ad50be08366f5316aa4854b77aff88))
* add discovery review and signed package catalog ([3738db5](https://github.com/Entigram-AI/entigram/commit/3738db5ad5efe4775268f7576c07cd092ef9b6b2))
* add discovery review and signed package catalog ([ec7acf3](https://github.com/Entigram-AI/entigram/commit/ec7acf3b570a8f5aafd84ba358041871922d2144))
* add etg merge ([f7088f6](https://github.com/Entigram-AI/entigram/commit/f7088f62b3e825640037f95fc92759d89bc5845a))
* add etg merge ([d35710d](https://github.com/Entigram-AI/entigram/commit/d35710d04f0944d106a3bbc181f63b06669c4e59))
* add expectation guard for pre-handoff agent verification ([41a125f](https://github.com/Entigram-AI/entigram/commit/41a125f6138eca23d29d50c66ee77fdff7b06ff3))
* add model gate feedback retries ([b1029bb](https://github.com/Entigram-AI/entigram/commit/b1029bbdb93b6367365d75fe2413ab88bb2cfd29))
* add model gate feedback retries ([a62a667](https://github.com/Entigram-AI/entigram/commit/a62a667ef5f557f04f88daec91b19435abf82a33))
* add modern Entigram logo assets ([83cf77c](https://github.com/Entigram-AI/entigram/commit/83cf77ce44fe7fd41aace1999b6fb2966aafc1ad))
* add modern Entigram logo assets ([f68e228](https://github.com/Entigram-AI/entigram/commit/f68e22813ef879b503ff5505e1573b220b6f726b))
* add Ollama launch option selection ([f40be78](https://github.com/Entigram-AI/entigram/commit/f40be78cf06c06a57e1e8c1e6b7c3548d1d23b67))
* add structured halt event json ([988dc12](https://github.com/Entigram-AI/entigram/commit/988dc120eecc7a10485a103c706f18bf0fef49d0))
* add structured halt event json ([0647e1c](https://github.com/Entigram-AI/entigram/commit/0647e1cecd29e2c1ac36f7859eaf0ee1df9a2108))
* add v2 action admission governance ([#45](https://github.com/Entigram-AI/entigram/issues/45)) ([f9d0fbf](https://github.com/Entigram-AI/entigram/commit/f9d0fbf3fc5953e4773c872f9363ec5cc25899fd))
* add WebSocket panel bridge for Agent-Hosted Panels ([07b85b3](https://github.com/Entigram-AI/entigram/commit/07b85b341bd3e5a8210e63321bb42294f8682306))
* add WebSocket panel bridge for Agent-Hosted Panels ([8527ee7](https://github.com/Entigram-AI/entigram/commit/8527ee73a48f6736ea0bad1344e91f7d3a4e7c65))
* add workspace usage and lifecycle controls ([2a5ece4](https://github.com/Entigram-AI/entigram/commit/2a5ece44dd0d39534183413b6bd7a79315c9cee2))
* add workspace usage and lifecycle controls ([7619ee9](https://github.com/Entigram-AI/entigram/commit/7619ee9315c77fbcf0b26bf194c1dad801832e32))
* add zero-trust MCP server ([d7d7e7a](https://github.com/Entigram-AI/entigram/commit/d7d7e7a240fe5e3df8582fe2933d5668813c4162))
* add zero-trust MCP server ([a949bc7](https://github.com/Entigram-AI/entigram/commit/a949bc73b02c992d0e91b73845dd14efb77efe54))
* auto-select Cloudflare coding models ([5ff4552](https://github.com/Entigram-AI/entigram/commit/5ff4552dcf0ada1906e1f2d443c44dd08d01a936))
* close indispensability gap — learnings, trust score, E2T mapping, session proposals ([9798f97](https://github.com/Entigram-AI/entigram/commit/9798f97d575493966a3e31f8167d6634bcb33697))
* commissioner companion features — evidence ledger, delivery snapshots, improvement proposals, Blocked state, CLI deliver/resolve/improve ([9bceedb](https://github.com/Entigram-AI/entigram/commit/9bceedb1e36e9711ab4c3c39d7a6bbf0e43d9824))
* deterministic ontology generation and change impact analysis ([39780f5](https://github.com/Entigram-AI/entigram/commit/39780f5036819753c9bae36b0f4fb1f040d11840))
* deterministic ontology generation and change impact analysis ([045fb10](https://github.com/Entigram-AI/entigram/commit/045fb10acae548dfcd707c23fb97ba8db3ff8e44))
* enforce adapters for workspace agents ([#37](https://github.com/Entigram-AI/entigram/issues/37)) ([9d1302b](https://github.com/Entigram-AI/entigram/commit/9d1302be631ee8f648d774496c76be2831f3daf7))
* enforce agent lifecycle check-ins ([#35](https://github.com/Entigram-AI/entigram/issues/35)) ([8320e21](https://github.com/Entigram-AI/entigram/commit/8320e2154dc1c8cf868e4c3182e405e7595332d3))
* GitHub issue links for package requests + demand tracking ([95b4605](https://github.com/Entigram-AI/entigram/commit/95b4605ad2834d0c866581b5da25c1f1279d9f42))
* governed security assessments, tech-aware advisories, and UI removal ([79a161b](https://github.com/Entigram-AI/entigram/commit/79a161b6334595b86ae6949eb19d00c39cf09de7))
* harden Cloudflare backend and agent governance ([9a68c83](https://github.com/Entigram-AI/entigram/commit/9a68c8320c925b8b9159f2a3a477e41950bed563))
* harden governed security assessment boundaries ([6d1fdc8](https://github.com/Entigram-AI/entigram/commit/6d1fdc8e79146ef4e16c9b4fb24e8496b729d926))
* harden governed security assessment boundaries ([de42a50](https://github.com/Entigram-AI/entigram/commit/de42a50bb1c07f4defa4f31d56e79ce6fb307934))
* harden MCP gate contract ([6828668](https://github.com/Entigram-AI/entigram/commit/6828668fced9e861b9218e653efccbd69cbf08e4))
* implement Cloud API package fetching using ENTIGRAM_TOKEN ([8bb6626](https://github.com/Entigram-AI/entigram/commit/8bb6626af19931e5b5688290fe873443549ef854))
* implement Cloud API package fetching using ENTIGRAM_TOKEN ([41783c4](https://github.com/Entigram-AI/entigram/commit/41783c4125f13dc4c4b558f7aadfe50926ece8eb))
* improve agent discoverability ([#34](https://github.com/Entigram-AI/entigram/issues/34)) ([ce270d6](https://github.com/Entigram-AI/entigram/commit/ce270d6bb2813e5bd59343904f2e069f9e544e2c))
* inject RelationalAlgebraGuard into etg_propose_alignment MCP handler ([090bb2c](https://github.com/Entigram-AI/entigram/commit/090bb2cd0e8a8e5b447f09a8f728be67cd39f0ef))
* install multi-agent PR review skill (moltenbits/claude-review pattern) ([b1eb279](https://github.com/Entigram-AI/entigram/commit/b1eb279d2cc6dddd60f5600aedf626c0f90d8e81))
* make agent governance portable ([b152b20](https://github.com/Entigram-AI/entigram/commit/b152b209b089b95e347b7bc75d25e1966db93a2a))
* make hydrate concise by default ([2b6fe0c](https://github.com/Entigram-AI/entigram/commit/2b6fe0cfda454a12b5c0100a70bf0f1d578e1a88))
* make hydrate concise by default ([29c0ea2](https://github.com/Entigram-AI/entigram/commit/29c0ea2c0157cdc4353015138cec40f7b1b80578))
* model canonical cross-agent governance policy ([fb30227](https://github.com/Entigram-AI/entigram/commit/fb30227f0bffd8cf71d6f2e54e3e82ab9f4199b5))
* out-of-the-box expectation guard for pre-handoff agent verification ([e558d63](https://github.com/Entigram-AI/entigram/commit/e558d6358cf4be4480f436b721fe6af01af4e7d5))
* package 6 multi-agent skills with entigram ([a53739b](https://github.com/Entigram-AI/entigram/commit/a53739b1be833b69f14aff3c1455cd414b74d141))
* publish community packages through standard registry ([#27](https://github.com/Entigram-AI/entigram/issues/27)) ([ac1a6d2](https://github.com/Entigram-AI/entigram/commit/ac1a6d26d0a68fdc97b6da3c9a41a8f41e074833))
* publish complete MCP tool contracts ([#44](https://github.com/Entigram-AI/entigram/issues/44)) ([8d0b798](https://github.com/Entigram-AI/entigram/commit/8d0b798c0c86b19428e4e4ca8ea1c1a5705516e9))
* security assessments, multi-agent skills, and package request flow ([caf389c](https://github.com/Entigram-AI/entigram/commit/caf389c3e6c8573aae693c5f25c8a98b55e9eddb))
* sign audit bundles and clarify versions ([131bddf](https://github.com/Entigram-AI/entigram/commit/131bddff5fdc2e7f9b9c00088d84bb9481722813))


### Bug Fixes

* add debug logging to suppression loader instead of silent swallow ([abe4742](https://github.com/Entigram-AI/entigram/commit/abe4742a3f5b2ed1144461dd50b1f4f3bd25f382))
* address P0-P2 findings from multi-agent review + rewrite skill ([c205276](https://github.com/Entigram-AI/entigram/commit/c2052762cacb818d6b9ccc0592dd9d2283b7301a))
* address P1/P2 findings from multi-agent review ([b4b124d](https://github.com/Entigram-AI/entigram/commit/b4b124d23eb7f6b36d324de014ababae2bd470e5))
* align Homebrew formula Python runtime ([7430531](https://github.com/Entigram-AI/entigram/commit/7430531a4fc27d923380e8f741b261cad1945da2))
* align Homebrew formula Python runtime ([615cbd2](https://github.com/Entigram-AI/entigram/commit/615cbd27b6cd53e57c57c00d6acd410dae300e0e))
* align Python support with MCP dependency ([f6e2309](https://github.com/Entigram-AI/entigram/commit/f6e2309bbb6a5050df07a1fb7b0179367b3f453f))
* align tests with federated routing guards ([f365811](https://github.com/Entigram-AI/entigram/commit/f365811b727dadf340f1a2b9012d198c5c93d32f))
* align tests with federated routing guards ([4999f87](https://github.com/Entigram-AI/entigram/commit/4999f87f441f443a908521cd06aa2af321bda30c))
* always raise flags — elevate advisories to warning, add suppression with rationale ([c5b9b81](https://github.com/Entigram-AI/entigram/commit/c5b9b8102d5d09e4d2aac67bc238a84d5f436a7f))
* avoid Claude slash model interception ([1885cff](https://github.com/Entigram-AI/entigram/commit/1885cffb5754bdb1afd0093e3e2c0398acc3631e))
* broaden homebrew formula detection ([15149c5](https://github.com/Entigram-AI/entigram/commit/15149c5b40594406515731ca4bea9c33c963154e))
* broaden homebrew formula detection ([f17eb13](https://github.com/Entigram-AI/entigram/commit/f17eb1349e53a06a50fc927634a2e2655cb5ed73))
* close CodeQL input handling alerts ([e4455ce](https://github.com/Entigram-AI/entigram/commit/e4455ce605003687ecc9401590f38a2bd2af03f3))
* close governed delivery review gaps ([23bc947](https://github.com/Entigram-AI/entigram/commit/23bc9476672a98c6c476cebb43feee8196483561))
* correct pre-handoff governance order ([056e009](https://github.com/Entigram-AI/entigram/commit/056e0094c14eb7875467137c2c5d59296ff57f54))
* direct access requests to developer@entigram.com ([094f1f8](https://github.com/Entigram-AI/entigram/commit/094f1f86459b3f4ddbfed09088871fd09cd6cb98))
* dynamically read version from pyproject.toml in tests ([ed07091](https://github.com/Entigram-AI/entigram/commit/ed07091eb98e50e4d1f98480904951ae38167e42))
* eliminate cryptography dependency for Homebrew builds ([7604ebb](https://github.com/Entigram-AI/entigram/commit/7604ebbce5b23aef39e0cf3bfd6d2a6f2b9bed8e))
* eliminate PyPI index race condition in homebrew release script ([04adeaa](https://github.com/Entigram-AI/entigram/commit/04adeaa6eb6100b900be4d3a8e563e9828812c94))
* emit proactive security advisories for detected workspace technologies ([2c80f5a](https://github.com/Entigram-AI/entigram/commit/2c80f5a2b903b4fc0851a893b8ad055006e4b50e))
* filter cryptography from poet resources and inject Homebrew dependency ([3197f1b](https://github.com/Entigram-AI/entigram/commit/3197f1b84ff38a9a091c2bd145b23ca65a60ac6a))
* filter cryptography from poet resources and inject Homebrew dependency ([ebda4d1](https://github.com/Entigram-AI/entigram/commit/ebda4d16b27cdf090a681f1c82a68e29efcd339d))
* filter pydantic and rpds-py rust dependencies ([a3f7aa2](https://github.com/Entigram-AI/entigram/commit/a3f7aa24bf3c2b078c0c8c1a5467addcc7699634))
* filter pydantic and rpds-py rust dependencies ([1ff8234](https://github.com/Entigram-AI/entigram/commit/1ff8234a7d9752394cdec7ba24913a1be5d06dab))
* generate brew-style-compliant formulae ([393a54f](https://github.com/Entigram-AI/entigram/commit/393a54f4d31e8ad6424c3579df19d340ee4c911d))
* generate brew-style-compliant formulae ([38790b6](https://github.com/Entigram-AI/entigram/commit/38790b6b669bc8e7cd409a76208cbb5b1684bfdc))
* generate homebrew formula resources using poet ([858eb2a](https://github.com/Entigram-AI/entigram/commit/858eb2a87b01487b0c9df267f8e5423943befcb3))
* generate homebrew formula resources using poet ([6a30a5e](https://github.com/Entigram-AI/entigram/commit/6a30a5ef08121a8a804927ab8f6bab350aeb0d06))
* harden governed delivery paths ([8924bab](https://github.com/Entigram-AI/entigram/commit/8924bab73868ead0292349617fe110972a1708d5))
* harden governed delivery paths ([5eeea8f](https://github.com/Entigram-AI/entigram/commit/5eeea8f1b8bd780ec1ac883c8edf8dae4811d46b))
* harden Homebrew resource filtering ([80363e6](https://github.com/Entigram-AI/entigram/commit/80363e65e850c747e5e45d5cc46417007653e9d6))
* harden impact analysis and alignment precedence ([e4b4f31](https://github.com/Entigram-AI/entigram/commit/e4b4f31d83e14d5e6ccd3c96e613dc06b73dde41))
* include setuptools in Homebrew resources ([5a8d7fa](https://github.com/Entigram-AI/entigram/commit/5a8d7fa53435362b53a59230f386babc3844d224))
* integrate assess into hydration, expand frameworks beyond OWASP, add package recs ([ee92c7e](https://github.com/Entigram-AI/entigram/commit/ee92c7ea0a13783776fedb3a955d3615cedca80b))
* make adapter fields optional in catalog audit for domain-only packages ([4152f8a](https://github.com/Entigram-AI/entigram/commit/4152f8a8725a3b325282e05548b97cd89f5c7a88))
* make adapter_module, source_kinds, adapters optional in catalog audit ([eac6887](https://github.com/Entigram-AI/entigram/commit/eac6887534ffc803ab4386aff1dc4e86483a8fe6))
* make community package installs self-guiding ([#30](https://github.com/Entigram-AI/entigram/issues/30)) ([058b202](https://github.com/Entigram-AI/entigram/commit/058b20255b2b51e1de11d16c91fdb00cc01194c8))
* model release PR governance rules ([bc92081](https://github.com/Entigram-AI/entigram/commit/bc92081560845df885260217deaab6e72764ee55))
* model release PR governance rules ([7dc2b67](https://github.com/Entigram-AI/entigram/commit/7dc2b67cba8cac3c4d458feb0927654204fa3f35))
* name initialized schema for Entigram ([#40](https://github.com/Entigram-AI/entigram/issues/40)) ([4ae679d](https://github.com/Entigram-AI/entigram/commit/4ae679d97c60fcee1f2a432d21f904ff7de1152d))
* normalize Cloudflare proxy message content ([d1914b0](https://github.com/Entigram-AI/entigram/commit/d1914b08f823e8c222c96d4eb4fc9686a8e3e37e))
* remove stale entigram.ui package refs breaking CI build ([27a77ac](https://github.com/Entigram-AI/entigram/commit/27a77acaed9f25dcce97d790ff8b156658ffbab6))
* remove vulnerable ecdsa dependency ([1bba9ab](https://github.com/Entigram-AI/entigram/commit/1bba9ab27d0bc91d68721541cba3e4d6372c8174))
* remove vulnerable ecdsa dependency ([e3a1763](https://github.com/Entigram-AI/entigram/commit/e3a17631d90e1d8d49582df4df7cea7785353907))
* replace cryptography with pure python ecdsa to resolve Homebrew sandbox build failures ([8728e89](https://github.com/Entigram-AI/entigram/commit/8728e893b0fb8c2de25e50d2c3685e79f893a4cd))
* resolve homebrew formula path dynamically ([4817f56](https://github.com/Entigram-AI/entigram/commit/4817f5618a66b500e251df4d4698d619eb848a60))
* resolve homebrew formula path dynamically ([4f64828](https://github.com/Entigram-AI/entigram/commit/4f64828a9fe09d3434ea8f13ca91ac4ca40ba97e))
* support pyproject-only release versioning ([2985462](https://github.com/Entigram-AI/entigram/commit/2985462a425191a6b9add0636518b3f57bc2de50))
* support pyproject-only release versioning ([eaeecee](https://github.com/Entigram-AI/entigram/commit/eaeecee22d58a9043086006f158d546d9aebd060))
* sync requirements.txt for CI build ([903b100](https://github.com/Entigram-AI/entigram/commit/903b100356b56c9c298fb9144cda63a794d16243))
* update hardcoded version 1.6.0 to 1.7.0 in tests ([aed13a8](https://github.com/Entigram-AI/entigram/commit/aed13a8e001e6464f05d961f5d2395260daa369c))
* update homebrew tap from pypi metadata ([4f3c5b2](https://github.com/Entigram-AI/entigram/commit/4f3c5b262e76cc96033106f816ba5128add66276))
* update homebrew tap from pypi metadata ([9c3a668](https://github.com/Entigram-AI/entigram/commit/9c3a668e364a00b13406d04f2192d045d5fe3799))
* use local repo root to avoid PyPI index race condition ([d2b329b](https://github.com/Entigram-AI/entigram/commit/d2b329b0b6cb687b4eb68c497120ab94afad8ae7))
* wait for PyPI sdist metadata ([108e19b](https://github.com/Entigram-AI/entigram/commit/108e19b0dda50c50bc8ae15fe89417ce04a6c168))
* wait for PyPI sdist metadata ([8fe7b28](https://github.com/Entigram-AI/entigram/commit/8fe7b28de9a3d545b04ebfc989de86ea044f7b4c))


### Documentation

* add M8ven verification badge ([#42](https://github.com/Entigram-AI/entigram/issues/42)) ([8f4e841](https://github.com/Entigram-AI/entigram/commit/8f4e8418c15e7358ed6044fc80333808766c1ba0))
* add opencode integration guidance ([2ccedcd](https://github.com/Entigram-AI/entigram/commit/2ccedcdabae57fbe115671363aab3fafc8af1789))
* add OpenCode integration guidance ([b09706c](https://github.com/Entigram-AI/entigram/commit/b09706c273aa569c13d5ea6ac6be17b2f906907b))
* add workspace alignment check to agent initialization step ([1aa7ef8](https://github.com/Entigram-AI/entigram/commit/1aa7ef86c5a7a2a57a107ead3002ed42df8324da))
* clarify user and agent workflows ([#32](https://github.com/Entigram-AI/entigram/issues/32)) ([3cdd3ae](https://github.com/Entigram-AI/entigram/commit/3cdd3ae02a2e9d7f7b42b6b0d458b07f6503a9fd))
* deduplicate 1.11.0 changelog entry ([67c6a12](https://github.com/Entigram-AI/entigram/commit/67c6a127a3c4b84ed147a46815619c792710f9ee))
* publish Entigram workspace standard ([428c55b](https://github.com/Entigram-AI/entigram/commit/428c55bfe432a9ff55a6223811f942b0700b24b0))
* publish workspace standard ([6ed435d](https://github.com/Entigram-AI/entigram/commit/6ed435d4645bf7527ed9889d7c2180c928cbb138))

## [1.25.0](https://github.com/Entigram-AI/entigram/compare/v1.24.1...v1.25.0) (2026-08-13)


### Features

* publish complete MCP tool contracts ([#44](https://github.com/Entigram-AI/entigram/issues/44)) ([8d0b798](https://github.com/Entigram-AI/entigram/commit/8d0b798c0c86b19428e4e4ca8ea1c1a5705516e9))


### Documentation

* add M8ven verification badge ([#42](https://github.com/Entigram-AI/entigram/issues/42)) ([8f4e841](https://github.com/Entigram-AI/entigram/commit/8f4e8418c15e7358ed6044fc80333808766c1ba0))

## [1.24.1](https://github.com/Entigram-AI/entigram/compare/v1.24.0...v1.24.1) (2026-08-13)


### Bug Fixes

* name initialized schema for Entigram ([#40](https://github.com/Entigram-AI/entigram/issues/40)) ([4ae679d](https://github.com/Entigram-AI/entigram/commit/4ae679d97c60fcee1f2a432d21f904ff7de1152d))

## [1.24.0](https://github.com/Entigram-AI/entigram/compare/v1.23.0...v1.24.0) (2026-08-13)


### Features

* enforce adapters for workspace agents ([#37](https://github.com/Entigram-AI/entigram/issues/37)) ([9d1302b](https://github.com/Entigram-AI/entigram/commit/9d1302be631ee8f648d774496c76be2831f3daf7))

## [1.23.0](https://github.com/Entigram-AI/entigram/compare/v1.22.0...v1.23.0) (2026-08-13)


### Features

* enforce agent lifecycle check-ins ([#35](https://github.com/Entigram-AI/entigram/issues/35)) ([8320e21](https://github.com/Entigram-AI/entigram/commit/8320e2154dc1c8cf868e4c3182e405e7595332d3))

## [1.22.0](https://github.com/Entigram-AI/entigram/compare/v1.21.1...v1.22.0) (2026-08-03)


### Features

* improve agent discoverability ([#34](https://github.com/Entigram-AI/entigram/issues/34)) ([ce270d6](https://github.com/Entigram-AI/entigram/commit/ce270d6bb2813e5bd59343904f2e069f9e544e2c))


### Documentation

* clarify user and agent workflows ([#32](https://github.com/Entigram-AI/entigram/issues/32)) ([3cdd3ae](https://github.com/Entigram-AI/entigram/commit/3cdd3ae02a2e9d7f7b42b6b0d458b07f6503a9fd))

## [1.21.1](https://github.com/Entigram-AI/entigram/compare/v1.21.0...v1.21.1) (2026-08-01)


### Bug Fixes

* make community package installs self-guiding ([#30](https://github.com/Entigram-AI/entigram/issues/30)) ([058b202](https://github.com/Entigram-AI/entigram/commit/058b20255b2b51e1de11d16c91fdb00cc01194c8))

## [1.21.0](https://github.com/Entigram-AI/entigram/compare/v1.20.0...v1.21.0) (2026-08-01)


### Features

* publish community packages through standard registry ([#27](https://github.com/Entigram-AI/entigram/issues/27)) ([ac1a6d2](https://github.com/Entigram-AI/entigram/commit/ac1a6d26d0a68fdc97b6da3c9a41a8f41e074833))

## [1.20.0](https://github.com/Entigram-AI/entigram/compare/v1.19.0...v1.20.0) (2026-08-01)


### Features

* GitHub issue links for package requests + demand tracking ([95b4605](https://github.com/Entigram-AI/entigram/commit/95b4605ad2834d0c866581b5da25c1f1279d9f42))
* security assessments, multi-agent skills, and package request flow ([caf389c](https://github.com/Entigram-AI/entigram/commit/caf389c3e6c8573aae693c5f25c8a98b55e9eddb))


### Bug Fixes

* address P1/P2 findings from multi-agent review ([b4b124d](https://github.com/Entigram-AI/entigram/commit/b4b124d23eb7f6b36d324de014ababae2bd470e5))

## [1.19.0](https://github.com/Entigram-AI/entigram/compare/v1.18.0...v1.19.0) (2026-08-01)


### Features

* add assessment package catalog and request-access monetization hook ([243fcc5](https://github.com/Entigram-AI/entigram/commit/243fcc570486771c2221bf1e16fe3f7d304dff27))
* governed security assessments, tech-aware advisories, and UI removal ([79a161b](https://github.com/Entigram-AI/entigram/commit/79a161b6334595b86ae6949eb19d00c39cf09de7))
* install multi-agent PR review skill (moltenbits/claude-review pattern) ([b1eb279](https://github.com/Entigram-AI/entigram/commit/b1eb279d2cc6dddd60f5600aedf626c0f90d8e81))
* package 6 multi-agent skills with entigram ([a53739b](https://github.com/Entigram-AI/entigram/commit/a53739b1be833b69f14aff3c1455cd414b74d141))


### Bug Fixes

* add debug logging to suppression loader instead of silent swallow ([abe4742](https://github.com/Entigram-AI/entigram/commit/abe4742a3f5b2ed1144461dd50b1f4f3bd25f382))
* address P0-P2 findings from multi-agent review + rewrite skill ([c205276](https://github.com/Entigram-AI/entigram/commit/c2052762cacb818d6b9ccc0592dd9d2283b7301a))
* always raise flags — elevate advisories to warning, add suppression with rationale ([c5b9b81](https://github.com/Entigram-AI/entigram/commit/c5b9b8102d5d09e4d2aac67bc238a84d5f436a7f))
* direct access requests to developer@entigram.com ([094f1f8](https://github.com/Entigram-AI/entigram/commit/094f1f86459b3f4ddbfed09088871fd09cd6cb98))
* emit proactive security advisories for detected workspace technologies ([2c80f5a](https://github.com/Entigram-AI/entigram/commit/2c80f5a2b903b4fc0851a893b8ad055006e4b50e))
* integrate assess into hydration, expand frameworks beyond OWASP, add package recs ([ee92c7e](https://github.com/Entigram-AI/entigram/commit/ee92c7ea0a13783776fedb3a955d3615cedca80b))
* remove stale entigram.ui package refs breaking CI build ([27a77ac](https://github.com/Entigram-AI/entigram/commit/27a77acaed9f25dcce97d790ff8b156658ffbab6))

## [1.18.0](https://github.com/Entigram-AI/entigram/compare/v1.17.1...v1.18.0) (2026-07-31)


### Features

* harden governed security assessment boundaries ([6d1fdc8](https://github.com/Entigram-AI/entigram/commit/6d1fdc8e79146ef4e16c9b4fb24e8496b729d926))

## [1.17.1](https://github.com/Entigram-AI/entigram/compare/v1.17.0...v1.17.1) (2026-07-31)


### Bug Fixes

* generate brew-style-compliant formulae ([393a54f](https://github.com/Entigram-AI/entigram/commit/393a54f4d31e8ad6424c3579df19d340ee4c911d))
* generate brew-style-compliant formulae ([38790b6](https://github.com/Entigram-AI/entigram/commit/38790b6b669bc8e7cd409a76208cbb5b1684bfdc))

## [1.17.0](https://github.com/Entigram-AI/entigram/compare/v1.16.2...v1.17.0) (2026-07-30)


### Features

* add workspace usage and lifecycle controls ([2a5ece4](https://github.com/Entigram-AI/entigram/commit/2a5ece44dd0d39534183413b6bd7a79315c9cee2))
* add workspace usage and lifecycle controls ([7619ee9](https://github.com/Entigram-AI/entigram/commit/7619ee9315c77fbcf0b26bf194c1dad801832e32))

## [1.16.2](https://github.com/nyabutid/entigram/compare/v1.16.1...v1.16.2) (2026-07-28)


### Bug Fixes

* close governed delivery review gaps ([23bc947](https://github.com/nyabutid/entigram/commit/23bc9476672a98c6c476cebb43feee8196483561))
* harden governed delivery paths ([8924bab](https://github.com/nyabutid/entigram/commit/8924bab73868ead0292349617fe110972a1708d5))
* harden governed delivery paths ([5eeea8f](https://github.com/nyabutid/entigram/commit/5eeea8f1b8bd780ec1ac883c8edf8dae4811d46b))


### Documentation

* add opencode integration guidance ([2ccedcd](https://github.com/nyabutid/entigram/commit/2ccedcdabae57fbe115671363aab3fafc8af1789))
* add OpenCode integration guidance ([b09706c](https://github.com/nyabutid/entigram/commit/b09706c273aa569c13d5ea6ac6be17b2f906907b))

## [1.16.1](https://github.com/nyabutid/entigram/compare/v1.16.0...v1.16.1) (2026-07-27)


### Documentation

* publish Entigram workspace standard ([428c55b](https://github.com/nyabutid/entigram/commit/428c55bfe432a9ff55a6223811f942b0700b24b0))
* publish workspace standard ([6ed435d](https://github.com/nyabutid/entigram/commit/6ed435d4645bf7527ed9889d7c2180c928cbb138))

## [1.16.0](https://github.com/nyabutid/entigram/compare/v1.15.0...v1.16.0) (2026-07-18)


### Features

* make hydrate concise by default ([2b6fe0c](https://github.com/nyabutid/entigram/commit/2b6fe0cfda454a12b5c0100a70bf0f1d578e1a88))
* make hydrate concise by default ([29c0ea2](https://github.com/nyabutid/entigram/commit/29c0ea2c0157cdc4353015138cec40f7b1b80578))

## [1.15.0](https://github.com/nyabutid/entigram/compare/v1.14.0...v1.15.0) (2026-07-18)


### Features

* auto-select Cloudflare coding models ([5ff4552](https://github.com/nyabutid/entigram/commit/5ff4552dcf0ada1906e1f2d443c44dd08d01a936))
* harden Cloudflare backend and agent governance ([9a68c83](https://github.com/nyabutid/entigram/commit/9a68c8320c925b8b9159f2a3a477e41950bed563))
* make agent governance portable ([b152b20](https://github.com/nyabutid/entigram/commit/b152b209b089b95e347b7bc75d25e1966db93a2a))


### Bug Fixes

* avoid Claude slash model interception ([1885cff](https://github.com/nyabutid/entigram/commit/1885cffb5754bdb1afd0093e3e2c0398acc3631e))
* normalize Cloudflare proxy message content ([d1914b0](https://github.com/nyabutid/entigram/commit/d1914b08f823e8c222c96d4eb4fc9686a8e3e37e))

## [1.14.0](https://github.com/nyabutid/entigram/compare/v1.13.0...v1.14.0) (2026-07-09)


### Features

* add WebSocket panel bridge for Agent-Hosted Panels ([07b85b3](https://github.com/nyabutid/entigram/commit/07b85b341bd3e5a8210e63321bb42294f8682306))
* add WebSocket panel bridge for Agent-Hosted Panels ([8527ee7](https://github.com/nyabutid/entigram/commit/8527ee73a48f6736ea0bad1344e91f7d3a4e7c65))

## [1.13.0](https://github.com/nyabutid/entigram/compare/v1.12.0...v1.13.0) (2026-07-05)


### Features

* add agent orchestration hibernation ([cc58dc4](https://github.com/nyabutid/entigram/commit/cc58dc44694bf7c872a557fe43f28cd2743b776a))
* add agent orchestration hibernation ([5e5b353](https://github.com/nyabutid/entigram/commit/5e5b353b3dead4ae622949493a9e70df822e92e5))

## [1.12.0](https://github.com/nyabutid/entigram/compare/v1.11.0...v1.12.0) (2026-07-05)


### Features

* add model gate feedback retries ([c56d659](https://github.com/nyabutid/entigram/commit/c56d6595abb2bccbc25bc489661072ff8d6ebfad))
* add model gate feedback retries ([8c9a3e4](https://github.com/nyabutid/entigram/commit/8c9a3e460f45b697a815507a1a46339390bde99e))
* add structured halt event json ([bbf8bef](https://github.com/nyabutid/entigram/commit/bbf8bef47bb104d3ba8bd6c1d970d59af6385b4f))
* add structured halt event json ([5ffcc17](https://github.com/nyabutid/entigram/commit/5ffcc173e6e582b50029a25958ae1390f29bf6bc))

## [1.11.0](https://github.com/nyabutid/entigram/compare/v1.10.1...v1.11.0) (2026-07-05)


### Features

* add etg merge ([30a31dd](https://github.com/nyabutid/entigram/commit/30a31dd14e88076bf3017a052518ccf26215357a))

## [1.10.1](https://github.com/nyabutid/entigram/compare/v1.10.0...v1.10.1) (2026-07-03)


### Bug Fixes

* make adapter fields optional in catalog audit for domain-only packages ([7b31865](https://github.com/nyabutid/entigram/commit/7b318657f434fa847c2220c64489af1afb9e35d2))
* make adapter_module, source_kinds, adapters optional in catalog audit ([f011e8f](https://github.com/nyabutid/entigram/commit/f011e8fdb909a79f164b49f287b71d0763b75385))

## [1.10.0](https://github.com/nyabutid/entigram/compare/v1.9.0...v1.10.0) (2026-07-03)


### Features

* add discovery review and signed package catalog ([6abed8f](https://github.com/nyabutid/entigram/commit/6abed8f237029a4d7558022f18e3f89cab51ff8d))
* add discovery review and signed package catalog ([a55c781](https://github.com/nyabutid/entigram/commit/a55c78172b3c5a55577d429916b08afcd27b4990))

## [1.9.0](https://github.com/nyabutid/entigram/compare/v1.8.0...v1.9.0) (2026-06-30)


### Features

* add Cloudflare Claude proxy launcher ([8076678](https://github.com/nyabutid/entigram/commit/8076678dae3a1732584ae23b8e394733658562b8))
* add Cloudflare Claude proxy launcher ([6d2aedb](https://github.com/nyabutid/entigram/commit/6d2aedb6e6e50a3d30e1596d943c7ae411fe9e4d))

## [1.8.0](https://github.com/nyabutid/entigram/compare/v1.7.8...v1.8.0) (2026-06-21)


### Features

* implement Cloud API package fetching using ENTIGRAM_TOKEN ([17f7ecf](https://github.com/nyabutid/entigram/commit/17f7ecf0fbb30af484e21a57746e24d8c9e679a7))
* implement Cloud API package fetching using ENTIGRAM_TOKEN ([83e6f84](https://github.com/nyabutid/entigram/commit/83e6f840b26629190856c16e415e2db7f3226fe1))

## [1.7.8](https://github.com/nyabutid/entigram/compare/v1.7.7...v1.7.8) (2026-06-21)


### Bug Fixes

* align Homebrew formula Python runtime ([fd10640](https://github.com/nyabutid/entigram/commit/fd1064089fb4d6deed718eaad47e767710365689))
* align Homebrew formula Python runtime ([7da8056](https://github.com/nyabutid/entigram/commit/7da80568c31f52d66853199e1b845b4d027a762a))

## [1.7.7](https://github.com/nyabutid/entigram/compare/v1.7.6...v1.7.7) (2026-06-21)


### Bug Fixes

* include setuptools in Homebrew resources ([2e364e4](https://github.com/nyabutid/entigram/commit/2e364e463e8d9e07b86f1a75f2b20f38a22aff3c))
* remove vulnerable ecdsa dependency ([11e5fa7](https://github.com/nyabutid/entigram/commit/11e5fa7cf002c7767e902d19806429623ed376cc))
* remove vulnerable ecdsa dependency ([8e5df53](https://github.com/nyabutid/entigram/commit/8e5df53b5e6621be5bc15f97dc466d034a88a14f))

## [1.7.6](https://github.com/nyabutid/entigram/compare/v1.7.5...v1.7.6) (2026-06-21)


### Bug Fixes

* wait for PyPI sdist metadata ([cead15e](https://github.com/nyabutid/entigram/commit/cead15e25ffb4ef12973175c63b391081ffd0176))
* wait for PyPI sdist metadata ([c835b42](https://github.com/nyabutid/entigram/commit/c835b42df0083316970b8011dc1a61314b526e85))

## [1.7.5](https://github.com/nyabutid/entigram/compare/v1.7.4...v1.7.5) (2026-06-21)


### Bug Fixes

* filter pydantic and rpds-py rust dependencies ([a608216](https://github.com/nyabutid/entigram/commit/a6082166576994fa1efbe80750c9cc4a7d429678))
* filter pydantic and rpds-py rust dependencies ([68478a7](https://github.com/nyabutid/entigram/commit/68478a7c3d79f9c8c118e97222613d6e96e76238))
* harden Homebrew resource filtering ([4c20ebd](https://github.com/nyabutid/entigram/commit/4c20ebdefd496ca8849930a4c9896296ba5759d0))

## [1.7.4](https://github.com/nyabutid/entigram/compare/v1.7.3...v1.7.4) (2026-06-21)


### Bug Fixes

* filter cryptography from poet resources and inject Homebrew dependency ([24b3d65](https://github.com/nyabutid/entigram/commit/24b3d65ddbffa076af7fda741f888b83553d62a1))
* filter cryptography from poet resources and inject Homebrew dependency ([6c5a8df](https://github.com/nyabutid/entigram/commit/6c5a8df2669364d4608fa4b2ccf451fef6d04749))

## [1.7.3](https://github.com/nyabutid/entigram/compare/v1.7.2...v1.7.3) (2026-06-21)


### Bug Fixes

* eliminate cryptography dependency for Homebrew builds ([572e8ad](https://github.com/nyabutid/entigram/commit/572e8ad3571384b4ae8723e5a16ac42df9a5b693))
* replace cryptography with pure python ecdsa to resolve Homebrew sandbox build failures ([2dee694](https://github.com/nyabutid/entigram/commit/2dee69408d490f9050ea1a359287048c5ca26ad5))

## [1.7.2](https://github.com/nyabutid/entigram/compare/v1.7.1...v1.7.2) (2026-06-21)


### Bug Fixes

* eliminate PyPI index race condition in homebrew release script ([a211b37](https://github.com/nyabutid/entigram/commit/a211b379683b6adf852100d1d4435a5f80394ca3))
* use local repo root to avoid PyPI index race condition ([0e95d74](https://github.com/nyabutid/entigram/commit/0e95d7420f6684b5afc4ed5c3ceb02d4cdbf44b4))

## [1.7.1](https://github.com/nyabutid/entigram/compare/v1.7.0...v1.7.1) (2026-06-21)


### Bug Fixes

* generate homebrew formula resources using poet ([01ecd5e](https://github.com/nyabutid/entigram/commit/01ecd5e1cb54547fc8ea75643c20595f0cf99819))
* generate homebrew formula resources using poet ([1173fa7](https://github.com/nyabutid/entigram/commit/1173fa77e0e3b7fe926e4a1273aeebcb9981ebad))

## [1.7.0](https://github.com/nyabutid/entigram/compare/v1.6.0...v1.7.0) (2026-06-21)


### Features

* sign audit bundles and clarify versions ([be12d03](https://github.com/nyabutid/entigram/commit/be12d0357b1098abe7426b534bbc10a9c3c208e3))


### Bug Fixes

* dynamically read version from pyproject.toml in tests ([309c43d](https://github.com/nyabutid/entigram/commit/309c43d89e0314e6a08d1eb519678ece52bf681d))
* sync requirements.txt for CI build ([cf01b76](https://github.com/nyabutid/entigram/commit/cf01b760ff4e1a0a2e60f06d49ced56e97e2906d))
* update hardcoded version 1.6.0 to 1.7.0 in tests ([4c5c7d9](https://github.com/nyabutid/entigram/commit/4c5c7d9be24a5c05e72844c43b68ca0b5c6b8c0b))

## [1.6.0](https://github.com/nyabutid/entigram/compare/v1.5.0...v1.6.0) (2026-06-21)

### Features

* add audit bundle and maintainer docs ([9c4840e](https://github.com/nyabutid/entigram/commit/9c4840ef4d037c6fcfead86101505283c0f88842))
* Entigram 1.6 introduces signed audit bundles for portable governance evidence.
* Document the MCP gate contract for `etg_get_schemas`, `etg_propose_alignment`, and `etg_log_conflict`.
* Add a deterministic Immutable Gate smoke test for schema discovery, hallucination rejection, ledger writes, delivery anchoring, and audit export.
* Keep the CLI/MCP runtime headless by default while publishing an optional Streamlit UI extra.

## [1.5.0](https://github.com/nyabutid/entigram/compare/v1.4.1...v1.5.0) (2026-06-20)


### Features

* deterministic ontology generation and change impact analysis ([dad7c8d](https://github.com/nyabutid/entigram/commit/dad7c8d9bf0f55813f7f58488361ee5580cfe85a))
* deterministic ontology generation and change impact analysis ([38ae72f](https://github.com/nyabutid/entigram/commit/38ae72f1936a2bc586df37907d311f844e465a6a))
* harden MCP gate contract ([4c52320](https://github.com/nyabutid/entigram/commit/4c5232011b9c0735310a544c67a2495d36476690))
* inject RelationalAlgebraGuard into etg_propose_alignment MCP handler ([af00ada](https://github.com/nyabutid/entigram/commit/af00ada789e1e3eaa9adbbb65228959dd11df0fb))
* model canonical cross-agent governance policy ([b4b480e](https://github.com/nyabutid/entigram/commit/b4b480ed85e6d8c95d0f9c90f3fc8ad343d75ab3))


### Bug Fixes

* correct pre-handoff governance order ([cb0aa04](https://github.com/nyabutid/entigram/commit/cb0aa04764294b33526e862ea8c269163c03a596))
* harden impact analysis and alignment precedence ([98880ca](https://github.com/nyabutid/entigram/commit/98880ca7d62d98425380bd65cb90d8c1dc8387b7))

## [1.4.1](https://github.com/nyabutid/entigram/compare/v1.4.0...v1.4.1) (2026-06-20)


### Bug Fixes

* support pyproject-only release versioning ([2ecc1cf](https://github.com/nyabutid/entigram/commit/2ecc1cfd7f304a3d8aeed8021e4321acaf4c3195))
* support pyproject-only release versioning ([d667780](https://github.com/nyabutid/entigram/commit/d667780ed2d94c1e051acf1693a4dd0e0a1e5432))

## [1.4.0](https://github.com/nyabutid/entigram/compare/v1.3.3...v1.4.0) (2026-06-20)


### Features

* add zero-trust MCP server ([0c5e3a3](https://github.com/nyabutid/entigram/commit/0c5e3a358757239dacea081936a2abae62b1379f))
* add zero-trust MCP server ([ee6f09c](https://github.com/nyabutid/entigram/commit/ee6f09c3fb4c89da41e3eecc8ca7cf952c31f98d))

## [1.3.3](https://github.com/nyabutid/entigram/compare/v1.3.2...v1.3.3) (2026-06-18)


### Bug Fixes

* update homebrew tap from pypi metadata ([2851af3](https://github.com/nyabutid/entigram/commit/2851af39c860a83748b0c96f173297f32dc938a1))
* update homebrew tap from pypi metadata ([adc332f](https://github.com/nyabutid/entigram/commit/adc332f832879ce5aa0e9d8727ecfd342f4790f3))

## [1.3.2](https://github.com/nyabutid/entigram/compare/v1.3.1...v1.3.2) (2026-06-18)


### Bug Fixes

* model release PR governance rules ([4360c02](https://github.com/nyabutid/entigram/commit/4360c02ef375c9b49186bbd9aa574ce4f47626e7))
* model release PR governance rules ([98bbbdd](https://github.com/nyabutid/entigram/commit/98bbbdd73a99ebb625592ebe64fd3ee4539dc1cb))

## [0.3.0](https://github.com/nyabutid/entigram/compare/v0.2.2...v0.3.0) (2026-06-04)


### Features

* add expectation guard for pre-handoff agent verification ([562dd72](https://github.com/nyabutid/entigram/commit/562dd72107d49b13f51d951cbaa75615222e2ded))
* out-of-the-box expectation guard for pre-handoff agent verification ([10d2471](https://github.com/nyabutid/entigram/commit/10d24712d5b79734a0f9205ea8fd6d9b30bde108))

## [0.2.2](https://github.com/nyabutid/entigram/compare/v0.2.1...v0.2.2) (2026-06-04)


### Bug Fixes

* broaden homebrew formula detection ([171671e](https://github.com/nyabutid/entigram/commit/171671e5e031d419b7c393209092edf4d8f59761))
* broaden homebrew formula detection ([f4fede4](https://github.com/nyabutid/entigram/commit/f4fede43d4a8ca9de94ca659e67fff20cd399b41))

## [0.2.1](https://github.com/nyabutid/entigram/compare/v0.2.0...v0.2.1) (2026-06-04)


### Bug Fixes

* resolve homebrew formula path dynamically ([c88adbd](https://github.com/nyabutid/entigram/commit/c88adbd3eaaf8423f28d4e63bc5fc489dd0abb87))
* resolve homebrew formula path dynamically ([5e3709c](https://github.com/nyabutid/entigram/commit/5e3709c9b6b35443f63c47fc00a055419fd41e21))

## [0.2.0](https://github.com/nyabutid/entigram/compare/v0.1.0...v0.2.0) (2026-06-04)


### Features

* add modern Entigram logo assets ([dab8f99](https://github.com/nyabutid/entigram/commit/dab8f9925a69418c849d7235aaf249e301cc6897))
* add modern Entigram logo assets ([43daa34](https://github.com/nyabutid/entigram/commit/43daa349b56fc7452aebe8311643ce987280dd13))
* add Ollama launch option selection ([3853b95](https://github.com/nyabutid/entigram/commit/3853b95cef8ecf0d2c1a0b23d21318b9eb7c7c83))
* close indispensability gap — learnings, trust score, E2T mapping, session proposals ([3203725](https://github.com/nyabutid/entigram/commit/3203725d6f4d797fa490122c0c945e74c7c04df3))
* commissioner companion features — evidence ledger, delivery snapshots, improvement proposals, Blocked state, CLI deliver/resolve/improve ([5eb7447](https://github.com/nyabutid/entigram/commit/5eb7447800e52ad944608c96b9db5cb777cb4a6a))


### Documentation

* add workspace alignment check to agent initialization step ([3e6204f](https://github.com/nyabutid/entigram/commit/3e6204f6f0783cf145482dad107b10edfe4a627d))

## 0.1.0 (2026-06-03)


### Bug Fixes

* align tests with federated routing guards ([2c8ed5e](https://github.com/nyabutid/entigram/commit/2c8ed5e6dd4ad246253535378cb8312256aefc6b))
* align tests with federated routing guards ([d48c969](https://github.com/nyabutid/entigram/commit/d48c969b887fcd55df9a76aa43a1bcaf69cfa393))

## [0.0.1](https://github.com/nyabutid/entigram/releases/tag/v0.0.1) (2026-06-02)

### Chore
* Initialize Entigram public baseline.
